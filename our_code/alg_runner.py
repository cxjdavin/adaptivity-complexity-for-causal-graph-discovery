...
from separator_no_check_policy import *
from r_adaptive_policy import *

ALG_DICT = {
    'adaptive_r1': (r_adaptive_policy, {'r': 1, 'k': 1}),
    'adaptive_r2': (r_adaptive_policy, {'r': 2, 'k': 1}),
    'adaptive_r3': (r_adaptive_policy, {'r': 3, 'k': 1}),
    'adaptive_rlogn': (r_adaptive_policy, {'r': 'logn', 'k': 1}),
    'adaptive_r2logn': (r_adaptive_policy, {'r': '2logn', 'k': 1}),
    'adaptive_r3logn': (r_adaptive_policy, {'r': '3logn', 'k': 1}),
    'adaptive_rn': (r_adaptive_policy, {'r': 'n', 'k': 1}),
    'separator': (separator_policy, {'k': 1}),
    'separator_no_check': (separator_no_check_policy, {'k': 1})
}
    ...
    def get_alg_results(self, overwrite=False, validate=True, multithread=True):
        ...
        if overwrite or not os.path.exists(self.alg_folder):
            ...
            def run_alg(tup):
                ...
                if 'r' in params:
                    if params['r'] == 'logn':
                        params['r'] = int(np.ceil(np.log2(dag.nnodes)))
                    if params['r'] == '2logn':
                        params['r'] = 2 * int(np.ceil(np.log2(dag.nnodes)))
                    if params['r'] == '3logn':
                        params['r'] = 3 * int(np.ceil(np.log2(dag.nnodes)))
                    if params['r'] == 'n':
                        params['r'] = dag.nnodes
