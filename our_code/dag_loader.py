    ...
    def get_dags(self, overwrite=False):
        ...
        if overwrite or not os.path.exists(self.dag_folder):
            ...
            while len(dags) < self.num_dags:
                ...
                elif self.sampler == DagSampler.HAIRBALL_PLUS_PROPORTIONAL: # Modified from HAIRBALL_PLUS
                    n = self.nnodes
                    deg = int(np.ceil(self.other_params['degree_prop'] * n))
                    e_min = int(np.ceil(self.other_params['e_min_prop'] * n))
                    e_max = int(np.ceil(self.other_params['e_max_prop'] * n))
                    d = DAG.from_nx(hairball_plus(deg, e_min, e_max,nnodes=n))
                elif self.sampler == DagSampler.GNP_TREE: # From [CS23]
                    n = self.nnodes
